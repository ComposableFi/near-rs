...
pub fn reconstruct_light_client_block_view_fields(
	...
	let approval_message = [
		...
		(block_view.inner_lite.height + 2).to_le().try_to_vec().unwrap(),
	]
	.concat();
...
pub struct ExecutionOutcomeViewForLiteClient {
...
impl From<ExecutionOutcomeView> for ExecutionOutcomeViewForLiteClient {
	fn from(view: ExecutionOutcomeView) -> Self {
		Self {
			logs: view.logs,
			receipt_ids: view.receipt_ids,
			gas_burnt: view.gas_burnt,
			tokens_burnt: view.tokens_burnt,
			executor_id: view.executor_id,
			status: view.status.try_to_vec().unwrap(),
		}
	}
}
...
mod tests {
	...
	fn parse_light_client_proof_response() {
		...
		assert_eq!(
			parsed_response.get_block_header_inner_lite_view().block_merkle_root.as_ref(),
			JSONCryptoHash::from(
		...
		assert_eq!(
			parsed_response.get_light_client_block_view().inner_rest_hash.as_ref(),
			JSONCryptoHash::from(
		...
		assert_eq!(
			parsed_response.get_light_client_block_view().prev_block_hash.as_ref(),
			JSONCryptoHash::from(
	...
	fn calculate_hash_light_client_block_lite_view() {
		...
		let current_block_hash =
			block_header_lite.get_light_client_block_view().current_block_hash();
