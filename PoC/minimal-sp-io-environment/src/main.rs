...
fn main() {
    ...
    ext.execute_with(|| {
        ...
        println!(
            "ECDSA Sig: {ecdsa_signature:?}, verified: {}",
            crypto::ecdsa_verify(&ecdsa_signature, msg, &ecdsa_key)
        );

        let ed25519_signature = crypto::ed25519_sign(id, &ed25519_key, msg).unwrap();
        println!(
            "ED25519 Sig: {ed25519_signature:?}, verified: {}",
            crypto::ed25519_verify(&ed25519_signature, msg, &ed25519_key)
        );

        let sr25519_signature = crypto::sr25519_sign(id, &sr25519_key, msg).unwrap();
        println!(
            "SR25519 Sig: {sr25519_signature:?}, verified: {}",
            crypto::sr25519_verify(&sr25519_signature, msg, &sr25519_key)
        );
    });
}
