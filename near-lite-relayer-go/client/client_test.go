...
func TestApprovalMessage(t *testing.T) {
	...
	approvalMessage, err := reconstrunctLightClientBlockViewFields(lightClientBLockView)
	require.Nil(t, err)
...
func TestValidatorStakeViewSerialization(t *testing.T) {
	lightClientBLockViewPreviousBlock, err := getLightClientBlockView(t, clientBlockResponsePreviousEpoch)
	require.Nil(t, err)
...
const clientBlockResponsePreviousEpoch = `
{
	"jsonrpc": "2.0",
	"result": {
		"approvals_after_next": [
...
const clientBlockResponseNextBlock = `
{
	"jsonrpc": "2.0",
	"result": {
		"approvals_after_next": [
...
func TestValidateAndUpdateHeadValidBlockNextEpoch(t *testing.T) {
	lightClientBLockViewPreviousEpoch, err := getLightClientBlockView(t, clientBlockResponsePreviousEpoch)
	require.Nil(t, err)

	lightClientBLockViewCurrentEpoch, err := getLightClientBlockView(t, clientBlockResponseNextBlock)
	require.Nil(t, err)
...
func TestSerializeRPCLightClientExecutionProofResponse(t *testing.T) {
	response := getRPCLightClientExecutionProofResponse([]byte(clientProofResponse))
...
func TestValidateTransaction(t *testing.T) {
	parsedResponse := getRPCLightClientExecutionProofResponse([]byte(clientProofResponse))
